...
return()=>window.removeEventListener('resize',updateCardsToShow);
    ...
    return (
        ...
        className='container mx-auto py-4 pt-20 px-6 md:px-20 lg:px-32 my-20  overflow-hidden' id='Projects'>
            ...
            <p className='text-center text-gray-500 mb-8 max-w-80 mx-auto'>Crafting spaces,Building Legacies-Explore Our Protofolio</p>

            <div className='flex justify-end items-center mb-8'>
               <button 
                   onClick={prevProject} 
                   className={`p-3 bg-gray-200 rounded mr-2 ${isFirstProject ? 'opacity-50 cursor-default' : 'cursor-pointer'}`} 
               ...
               <button 
                   onClick={nextProject} 
                   className={`p-3 bg-gray-200 rounded mr-2 ${isLastProject ? 'opacity-50 cursor-default' : 'cursor-pointer'}`}
                ...
                <div className='flex gap-8 transition-transform duration-500 ease-in-out' style={{transform:`translateX(-${(currentIndex * 100)/cardsToShow}%)`}}>
                    {projectsData.map((project,index)=>(
                        <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4'>
                            ...
                            <div className='inline-block bg-white/40 backdrop-blur-lg w-3/4 px-4 py-2 shadow-md'>
                                <h2 className='text-xl font-semibold text-gray-800 pl-4'>{project.title}</h2>
