  ...
  return (
    <div className='absolute top-0 left-0 w-full z-10'>
        ...
        <ul className='hidden md:flex gap-7 text-white'>
         ...
         <img onClick={()=>setShowMenu(true)} src={assets.menu_icon} className='md:hidden w-7 cursor-pointer' alt=''></img>
        </div>

    {/* -----mobile menu----- */}
        <div className={`md:hidden ${showMenu?'w-full ':'w-0 h-0'}right-0 top-0 bottom-0 overflow-hidden bg-white/40 backdrop-blur-20 transition-all`}>
            ...
            <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                ...
                <a  onClick={()=>setShowMenu(false)} href='#Testimonials' className='px-4 py-4 rounded-full inline-block'>Testimonials</a>
            </ul>
        </div>
    </div>
  )
